...
class VideoDiffusionModelWithCtrl(DiffusionV2WModel):
    ...
    def generate_samples_from_batch(
        ...
        if self.net.is_context_parallel_enabled:
            ...
            intermediates = cat_outputs_cp(intermediates, seq_dim=3, cp_group=self.net.cp_group) if SAVE_INTERMEDIATES else None
...
class VideoDiffusionT2VModelWithCtrl(DiffusionT2WModel):
    ...
    def generate_samples_from_batch(
        ...
        **kwargs,
        ...
        if self.net.is_context_parallel_enabled:
            samples = cat_outputs_cp(samples, seq_dim=2, cp_group=self.net.cp_group)
            intermediates = cat_outputs_cp(intermediates, seq_dim=2, cp_group=self.net.cp_group) if SAVE_INTERMEDIATES else None
        return samples, intermediates
